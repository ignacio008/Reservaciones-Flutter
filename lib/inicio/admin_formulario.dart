...
class AdminFormulario extends StatefulWidget {
  UsuarioModel iconmodelUsuario;
  AdminFormulario(this.iconmodelUsuario);

  @override
  State<AdminFormulario> createState() => _AdminFormularioState();
}

class _AdminFormularioState extends State<AdminFormulario> {
    ...
    final nombreFormulario= TextEditingController();
    
    final telefonoFormulario= TextEditingController();
    
    final correoFormulario= TextEditingController();

    final comentarioFormulario= TextEditingController();
                      ...
                      GestureDetector(
                        ...
                        TextFormField(
                                     ...
                                     keyboardType: TextInputType.multiline,
                                       maxLines: 3,
                                      controller:comentarioFormulario,
                                            decoration: InputDecoration(   
  ...
  void crearPublicidadUrl() async{
    ...
    String nombre_formulario=nombreFormulario.text.trim();
    String telefono_fomrulario=telefonoFormulario.text.trim();
    String correo_formulario=correoFormulario.text.trim();
    String comentario_formulario=comentarioFormulario.text.trim();
    String url_formulario="URL del url Formulario";
    String url_tienda="URL del url tienda";
    int tippPublicidad=1;
    String msnImagen="La imagen no puede estar vacia";
    String msnUrl="Verifique que el url este bien escrito";
     bool phoneValid = RegExp(r'^\d{10}$').hasMatch(telefono_fomrulario);
    ...
    if(url_publicidad_.isNotEmpty && validadURL==false){
      showToast(msnUrl);
      return;
    }
    if(!correo_formulario.contains("@") || correo_formulario.length < 3 ){
    showToast(msnToastEmail);
      return;
    }

    if(telefono_fomrulario != null && phoneValid==false){
